...
for Participant_Num = 1:N_Participants
    ...
    if Participant_Num < 10
        mkdir(strcat(['20220912_S0' num2str(Participant_Num) '_POD1']))
        cd(strcat(['20220912_S0' num2str(Participant_Num)  '_POD1']))
    else
        mkdir(strcat(['20220912_S' num2str(Participant_Num)  '_POD2']))
        cd(strcat(['20220912_S' num2str(Participant_Num)  '_POD2']))
    end
...
%     Noise_Amplitude = 0.02*rand(1,1) + 0.005;
    ...
    ax(Participant_Num) = subplot(ceil(sqrt(N_Participants)), ...
        ceil(sqrt(N_Participants)),Participant_Num); plot(SLA,'-');
    xlabel('Stride Number'), ylabel('Step Len. Asym')
    
    % Save each participant's data in a csv file
    if Participant_Num < 10
        writematrix(SLA', ...
            strcat(['20220912_S0' num2str(Participant_Num) 'POD1' '_SLA.csv']))
    else
        writematrix(SLA', ...
            strcat(['20220912_S' num2str(Participant_Num) 'POD2' 'SLA.csv']))
    end
    cd(Home)
end
linkaxes(ax)
