...
for Participant_Num = 1:N_Participants
    ...
    if Participant_Num < 10
        mkdir(strcat(['2023_10_20_S' num2str(Participant_Num) '_POD1']))
        cd(strcat(['2023_10_20_S' num2str(Participant_Num) '_POD1']))
    else
        mkdir(strcat(['2023_10_20_S' num2str(Participant_Num) '_POD2']))
        cd(strcat(['2023_10_20_S' num2str(Participant_Num) '_POD2']))
    end
    ...
    Noise_Amplitude = 0.08*rand(1,1) + 0.005;
    ...
    if Participant_Num < 10
        writematrix(SLA', ...
            strcat(['2023_10_20_S_' num2str(Participant_Num) '_POD1.csv']))
    else
        writematrix(SLA', ...
            strcat(['2023_10_20_S' num2str(Participant_Num) '_POD2.csv']))
    end
    cd(Home)
end
%linkaxes(ax)
...
% Activity Week 3
% What happened when you changed N?
    % More plots showed up as N increased
% Increase amount of noise generated per generated sample, what does this
% do?
    % Increases the variability of the data in the y direction.
% Change both folder name and csv file to different format
    % Done!
% Which line of code aligns values for the plot's y-axis?
    % linkaxes(ax)
